...
#define debug_println(lvl_ignored, fmt, ...) \
    do { \
       if (linker->message) linker->message(LDPL_INFO, fmt __VA_OPT__(,) ## __VA_ARGS__ ); \
...
#define member_functions(v, w) \
    ...
    v(enum ld_plugin_status, cleanup) \
...
#define declare_member_closure(rett, name, ...) \
	 ...
	 * virtual, since it's always called directly. */
#define LINKER_PLUGIN(plugin_t, onload) \
	extern "C" { \
